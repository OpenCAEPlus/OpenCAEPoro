                              ...
                              const OCP_DBL& dt) const
{
    ...
    for (USI p = 0; p < numPerf; p++) {
        ...
        for (USI j = 0; j < np; j++) {
            flagB[j] = myBulk.phaseExist[n * np + j];
        }
    ...
    for (USI p = 0; p < numPerf; p++) {
        ...
        for (USI j = 0; j < np; j++) {
            ...
            for (USI i = 0; i < nc; i++) {
                ...
                if (j == np - 1)  continue;  // skip water phase
                // dQ / dxij               
                for (USI k = 0; k < nch; k++) {
                    dQdXsB[(i + 1) * ncolB + npB + jxB * nch + k] +=
                        -transIJ * dP / mu * myBulk.mux[n_np_j * nc + k];
                }
            }
            jxB++;
    ...
    const OCP_DBL& dt) const
{
    ...
    for (USI p = 0; p < numPerf; p++) {
        ...
        for (USI j = 0; j < np; j++) {
            ...
            USI ib = 0; USI ie = nch;
            if (j == np - 1) { ib = nc - 1; ie = nc; } // water phase 
            for (USI i = ib; i < ie; i++) {
                xij = myBulk.xij[n_np_j * nc + i];
                // dQ / dP
                transIJ = perf[p].transj[j] * xi * xij;
                ...
                // dQ / dCij
                if (j == np - 1)  continue; // skip water

                for (USI k = 0; k < nch; k++) {
                    tmp = dP * perf[p].transj[j] * xij *
                        (myBulk.xix[n_np_j * nc + k] -
                            xi / mu * myBulk.mux[n_np_j * nc + k]);
                    dQdXsB[(i + 1) * ncolB + npB + jxB * nch + k] += tmp;

                    if (npB + jxB * nch + k >= ncolB)
                        cout << "get it" << endl;
                }
                dQdXsB[(i + 1) * ncolB + npB + jxB * nch + i] += perf[p].transj[j] * xi * dP;;
            }
            jxB++;
        }
        // Bulk to Well
...
void Well::AssembleMatINJ_FIM_new1(const Bulk& myBulk, LinearSystem& myLS,
    const OCP_DBL& dt) const
{
    OCP_FUNCNAME;
...
void Well::AssembleMatPROD_FIM_new1(const Bulk& myBulk, LinearSystem& myLS,
    const OCP_DBL& dt) const
{
    OCP_FUNCNAME;
    ...
    const OCP_DBL& dt) const
{
    ...
    for (USI p = 0; p < numPerf; p++) {
        ...
        for (USI j = 0; j < np; j++) {
            ...
            for (USI i = 0; i < nc; i++) {
                ...
                for (USI k = 0; k < np; k++) {
                    dQdXsB[(i + 1) * ncol2 + k] +=
                        CONV1 * perf[p].WI * perf[p].multiplier * perf[p].xi *
                        opt.zi[i] * myBulk.dKr_dS[pIde * np * np + j * np + k] * dP / mu;
                }
                // dQ / dxij
                for (USI k = 0; k < nc; k++) {
                    dQdXsB[(i + 1) * ncol2 + np + j * nc + k] +=
                        -transIJ * dP / mu * myBulk.mux[pIde_np_j * nc + k];
                }
            }
        }
        // Bulk to Well
        bmat = dQdXpB;
        DaABpbC(ncol, ncol, ncol2, 1, dQdXsB.data(), &myBulk.dSec_dPri[pIde * bsize2], 1,
            bmat.data());
        Dscalar(bsize, dt, bmat.data());
        // Add
        USI ptr = myLS.diagPtr[pIde];
        for (USI i = 0; i < bsize; i++) {
            myLS.val[pIde][ptr * bsize + i] += bmat[i];
        }
        // Insert
        bmat = dQdXpW;
        Dscalar(bsize, dt, bmat.data());
        myLS.val[pIde].insert(myLS.val[pIde].end(), bmat.begin(), bmat.end());
        myLS.colId[pIde].push_back(wId);
        ...
        case LRATE_MODE:
            ...
            bmat = dQdXpB;
            DaABpbC(ncol, ncol, ncol2, 1, dQdXsB.data(),
                &myBulk.dSec_dPri[pIde * bsize2], 1, bmat.data());
            fill(bmat2.begin(), bmat2.end(), 0.0);
            for (USI i = 0; i < nc; i++) {
                Daxpy(ncol, 1.0, bmat.data() + (i + 1) * ncol, bmat2.data());
            }
            myLS.val[wId].insert(myLS.val[wId].end(), bmat2.begin(), bmat2.end());
            myLS.colId[wId].push_back(pIde);
            break;
...
void Well::AssembleMatPROD_AIMt(const Bulk& myBulk, LinearSystem& myLS,
    const OCP_DBL& dt) const
{
    OCP_FUNCNAME;
    ...
    for (USI p = 0; p < numPerf; p++) {
        ...
        for (USI j = 0; j < np; j++) {
            ...
            for (USI i = 0; i < nc; i++) {
                ...
                for (USI k = 0; k < np; k++) {
                    tmp = CONV1 * perf[p].WI * perf[p].multiplier * dP / mu * xi * xij *
                        myBulk.dKr_dS[pIde * np * np + j * np + k];
                    // capillary pressure
                    tmp += transIJ * myBulk.dPcj_dS[pIde * np * np + j * np + k];
                    dQdXsB[(i + 1) * ncol2 + k] += tmp;
                }
                // dQ / dCij
                for (USI k = 0; k < nc; k++) {
                    tmp = dP * perf[p].transj[j] * xij *
                        (myBulk.xix[pIde_np_j * nc + k] -
                            xi / mu * myBulk.mux[pIde_np_j * nc + k]);
                    if (k == i) {
                        tmp += perf[p].transj[j] * xi * dP;
                    }
                    dQdXsB[(i + 1) * ncol2 + np + j * nc + k] += tmp;
...
void Well::AssembleMatINJ_AIMs(const Bulk& myBulk, LinearSystem& myLS,
    const OCP_DBL& dt) const
{
    OCP_FUNCNAME;
    ...
    for (USI p = 0; p < numPerf; p++) {
        OCP_USI n = perf[p].location;
        OCP_USI pId = myBulk.map_Bulk2FIM[n];
        OCP_ASSERT(pId < myBulk.numFIMBulk, "wrong FIM Bulk");
        ...
        for (USI j = 0; j < np; j++) {
            ...
            for (USI i = 0; i < nc; i++) {
                ...
                for (USI k = 0; k < np; k++) {
                    dQdXsB[(i + 1) * ncol2 + k] +=
                        CONV1 * perf[p].WI * perf[p].multiplier * perf[p].xi *
                        opt.zi[i] * myBulk.dKr_dS[pId * np * np + j * np + k] * dP / mu;
                }
                // dQ / dxij
                for (USI k = 0; k < nc; k++) {
                    dQdXsB[(i + 1) * ncol2 + np + j * nc + k] +=
        ...
        // Bulk to Well
        bmat = dQdXpB;
        DaABpbC(ncol, ncol, ncol2, 1, dQdXsB.data(), &myBulk.dSec_dPri[pId * bsize2], 1,
            bmat.data());
        Dscalar(bsize, dt, bmat.data());
        // Add
        USI ptr = myLS.diagPtr[n];
        ...
        case LRATE_MODE:
            ...
            bmat = dQdXpB;
            DaABpbC(ncol, ncol, ncol2, 1, dQdXsB.data(),
                &myBulk.dSec_dPri[pId * bsize2], 1, bmat.data());
            fill(bmat2.begin(), bmat2.end(), 0.0);
            for (USI i = 0; i < nc; i++) {
                Daxpy(ncol, 1.0, bmat.data() + (i + 1) * ncol, bmat2.data());
            }
...
void Well::AssembleMatPROD_AIMs(const Bulk& myBulk, LinearSystem& myLS,
    const OCP_DBL& dt) const
{
    OCP_FUNCNAME;
    ...
    for (USI p = 0; p < numPerf; p++) {
        ...
        for (USI j = 0; j < np; j++) {
            ...
            for (USI i = 0; i < nc; i++) {
                ...
                for (USI k = 0; k < np; k++) {
                    tmp = CONV1 * perf[p].WI * perf[p].multiplier * dP / mu * xi * xij *
                        myBulk.dKr_dS[pId * np * np + j * np + k];
                    // capillary pressure
                    tmp += transIJ * myBulk.dPcj_dS[pId * np * np + j * np + k];
                    dQdXsB[(i + 1) * ncol2 + k] += tmp;
                }
                // dQ / dCij
                for (USI k = 0; k < nc; k++) {
