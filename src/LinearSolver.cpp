...
int LinearSolver::BFaspSolve()
{
    ...
    if (solver_type >= 1 && solver_type <= 10) {
        ...
        switch (precond_type) {
            case PC_NULL:
                status =
                    fasp_solver_dbsr_krylov(&A_BFasp, &b_BFasp, &x_BFasp, &itParam);
                break;
            case PC_DIAG:
                status = fasp_solver_dbsr_krylov_diag(&A_BFasp, &b_BFasp, &x_BFasp,
                                                      &itParam);
                break;
            case PC_BILU:
                status = fasp_solver_dbsr_krylov_ilu(&A_BFasp, &b_BFasp, &x_BFasp,
                                                     &itParam, &iluParam);
                break;
            case PC_FASP1:
