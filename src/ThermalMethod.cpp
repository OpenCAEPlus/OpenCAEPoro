...
OCP_BOOL T_FIM::FinishNR(Reservoir& rs, OCPControl& ctrl)
{
    ...
    cout << "### NR : " + to_string(ctrl.iterNR) + "    Res:    " << setprecision(2)
        << scientific << rs.bulk.res.maxRelRes0_V << setw(12) << rs.bulk.res.maxRelRes_V
        << setw(12) << rs.bulk.res.maxRelRes_N << setw(12) << rs.bulk.res.maxWellRelRes_mol <<
        setw(12) << rs.bulk.res.maxRelRes_E << setw(12) << fabs(NRdPmax) << setw(12) << fabs(NRdSmax) << endl;
...
void T_FIM::AllocateLinearSystem(LinearSystem& ls, const Reservoir& rs, const OCPControl& ctrl)
{
...
void T_FIM::CalRock(Bulk& bk) const
{
    ...
    for (OCP_USI n = 0; n < nb; n++) {
        if (bk.bType[n] > 0) {
            // fluid exists
            bk.rock[bk.ROCKNUM[n]]->CalPoroT(bk.P[n], bk.T[n], bk.poroInit[n], bk.poro[n], bk.poroP[n],
                bk.poroT[n], vr, vrP, vrT);
...
void T_FIM::InitFlash(Bulk& bk) const
{
    ...
    for (OCP_USI n = 0; n < nb; n++) {
        if (bk.bType[n] > 0) {
            bk.flashCal[bk.PVTNUM[n]]->InitFlashFIM(bk.P[n], bk.Pb[n], bk.T[n], &bk.S[n * np],
                bk.rockVp[n], &bk.Ni[n * nc], n);
            for (USI i = 0; i < nc; i++) {
...
void T_FIM::CalFlash(Bulk& bk) const
{
    ...
    for (OCP_USI n = 0; n < nb; n++) {
        if (bk.bType[n] > 0) {
            bk.flashCal[bk.PVTNUM[n]]->FlashFIM(bk.P[n], bk.T[n], &bk.Ni[n * nc], &bk.S[n * np],
                bk.phaseNum[n], &bk.xij[n * np * nc], n);
            PassFlashValue(bk, n);
        }
    }
}
    ...
    for (USI j = 0; j < np; j++) {
        ...
        if (bk.phaseExist[bIdp + j]) {
            ...
            for (USI i = 0; i < nc; i++) {
                ...
                bk.Hx[bIdp * nc + j * nc + i] =
                    bk.flashCal[pvtnum]->GetHx(j, i);
            }
        }
    }
    ...
    for (OCP_USI n = 0; n < bk.numBulk; n++) {
        if (bk.bType[n] > 0) {
            ...
            bk.flow[bk.SATNUM[n]]->CalKrPcDeriv(&bk.S[bId], &bk.kr[bId], &bk.Pc[bId],
                &bk.dKr_dS[bId * np], &bk.dPcj_dS[bId * np], n);
            for (USI j = 0; j < np; j++)
...
void T_FIM::CalRes(Reservoir& rs, const OCP_DBL& dt, const OCP_BOOL& resetRes0)
{
    ...
    for (OCP_USI n = 0; n < nb; n++) {
        if (bk.bType[n] > 0) {
            ...
            // Energy Conservation
            Res.resAbs[n * len + nc + 1] = (bk.vf[n] * bk.Uf[n] + bk.vr[n] * bk.Hr[n])
    ...
    for (OCP_USI c = 0; c < conn.numConn; c++) {
        ...
        if (bk.bType[bId] > 0 && bk.bType[eId] > 0) {
            ...
            for (USI j = 0; j < np; j++) {
                ...
                const OCP_DBL tmpV = dt * conn.upblock_Velocity[c * np + j] 
                                   * bk.xi[uId_np_j];
    ...
    for (const auto& wl : rs.allWells.wells) {
        if (wl.IsOpen()) {
            ...
            if (wl.WellType() == INJ) { 
                ...
                const OCP_DBL Hw = bk.flashCal[0]->CalInjWellEnthalpy(wl.InjTemp(), &wl.InjZi()[0]);
                for (USI p = 0; p < wl.PerfNum(); p++) {
                    const OCP_USI k = wl.PerfLocation(p);
                    Res.resAbs[k * len + 1 + nc] += wl.PerfInjQt_ft3(p) * wl.PerfXi(p) * Hw * dt;
                }
                ...
                for (USI p = 0; p < wl.PerfNum(); p++) {
                    ...
                    // Mass Conservation
                    for (USI j = 0; j < np; j++) {
                        Res.resAbs[k * len + 1 + nc] += wl.PerfProdQj_ft3(p, j) * bk.xi[k * np + j] * bk.H[k * np + j] * dt;
                    }                   
                }
                             ...
                             const OCP_DBL&   dt) const
{
    ...
    for (OCP_USI n = 0; n < nb; n++) {
        if (bk.bType[n] > 0) {
            ...
            // Energy consevation
            // dP
            bmat[ncol * (ncol - 1)] = bk.vfP[n] * bk.Uf[n] +
                                      bk.vf[n] * bk.UfP[n] +
                                      bk.vrP[n] * bk.Hr[n];
            // dNi
            for (USI i = 0; i < nc; i++) {
                bmat[ncol * (ncol - 1) + i + 1] =
                    bk.vfi[n * nc + i] * bk.Uf[n] +
                    bk.vf[n] * bk.Ufi[n * nc + i];
            }
            // dT
            bmat[ncol * ncol - 1] =
    ...
    for (OCP_USI c = 0; c < conn.numConn; c++) {
        ...
        // Assemble
        bmat = dFdXpB;
        DaABpbC(ncol, ncol, ncol2, 1, dFdXsB.data(), &bk.dSec_dPri[bId * bsize2],
                1, bmat.data());
        ...
        // End
        bmat = dFdXpE;
        DaABpbC(ncol, ncol, ncol2, 1, dFdXsE.data(), &bk.dSec_dPri[eId * bsize2],
                1, bmat.data());
        Dscalar(bsize, dt, bmat.data());
        // Begin - End -- insert
...
void T_FIM::AssembleMatWells(LinearSystem& ls, const Reservoir& rs, const OCP_DBL& dt) const
{
...
void T_FIM::AssembleMatInjWells(LinearSystem& ls, const Bulk& bk, const Well& wl, const OCP_DBL& dt) const
{
...
void T_FIM::AssembleMatProdWells(LinearSystem& ls, const Bulk& bk, const Well& wl, const OCP_DBL& dt) const
{
    const USI np        = bk.numPhase;
    const USI nc        = bk.numCom; 
    const USI ncol      = nc + 2;
    ...
    for (USI p = 0; p < wl.PerfNum(); p++) {
        ...
        for (USI j = 0; j < np; j++) {
            ...
            // dQ / dP
            dQdXpB[(nc + 1) * ncol] +=  transJ * (1 - dP * muP / mu) * H +
                dP * wl.PerfTransj(p, j) * xiP * H;
            dQdXpW[(nc + 1) * ncol] += -transJ * H;

            // dQ / dT
            dQdXpB[(nc + 2) * ncol - 1] +=
                transJ * (-dP * muT / mu) * H +
                    dP * wl.PerfTransj(p, j) * xiT * H + transJ * dP * HT;
            dQdXpW[(nc + 2) * ncol - 1] += 0;
...
void T_FIM::GetSolution(Reservoir& rs, const vector<OCP_DBL>& u, const OCPControl& ctrl) const
{
