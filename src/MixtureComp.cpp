...
MixtureComp::MixtureComp(const EoSparam& param, const USI& tar)
{
	...
	Cname = param.Cname;
	if (param.Tc.activity)          Tc = param.Tc.data[tar];
	else                            OCP_ABORT("TCRIT hasn't been input!");
	if (param.Pc.activity)          Pc = param.Pc.data[tar];
	else                            OCP_ABORT("PCRIT hasn't been input!");
	
	if (param.Vc.activity)      Vc = param.Vc.data[tar];
	...
	if (param.MW.activity)          MWC = param.MW.data[tar];
	else                            OCP_ABORT("MW hasn't been input!");
	if (param.Acf.activity)         Acf = param.Acf.data[tar];
	else                            OCP_ABORT("ACF hasn't been input!");
	if (param.OmegaA.activity)      OmegaA = param.OmegaA.data[tar];
	else                            OmegaA.resize(NC, 0.457235529);
	if (param.OmegaB.activity)      OmegaB = param.OmegaB.data[tar];
	else                            OmegaB.resize(NC, 0.077796074);
	
	if (param.Vshift.activity)      Vshift = param.Vshift.data[tar];
	else                            Vshift.resize(NC, 0);
...
void MixtureComp::InitFlash(const OCP_DBL& Pin, const OCP_DBL& Pbbin, const OCP_DBL& Tin,
	...
	const OCP_DBL* Ziin)
{
	...
	Ni[Wcid] = Vpore * Sw * xi[Wpid];	Nt += Ni[Wcid];
...
void MixtureComp::FlashDeriv(const OCP_DBL& Pin, const OCP_DBL& Tin, const OCP_DBL* Niin)
{
	CalFlash(Pin, Tin, Niin);
	// Calculate derivates for hydrocarbon phase and components
	// d vf / d Ni, d vf / d P
...
OCP_DBL MixtureComp::XiPhase(const OCP_DBL& Pin, const OCP_DBL& Tin, const OCP_DBL* Ziin)
{
...
OCP_DBL MixtureComp::RhoPhase(const OCP_DBL& Pin, const OCP_DBL& Tin, const OCP_DBL* Ziin)
{ 
...
OCP_DBL MixtureComp::GammaPhaseOG(const OCP_DBL& Pin, const OCP_DBL& Tin, const OCP_DBL* Ziin)
{
...
void MixtureComp::CalAjBj(OCP_DBL& AjT, OCP_DBL& BjT, const vector<OCP_DBL>& xj) const
{
	...
	for (USI i1 = 0; i1 < NC; i1++) {
		...
		for (USI i2 = 0; i2 < i1; i2++) {
			AjT += 2 * xj[i1] * xj[i2] * sqrt(Ai[i1] * Ai[i2]) * (1 - BIC[i1 * NC + i2]);
		}
	}
}
...
void MixtureComp::CalFugNAll()
{
	...
	for (USI j = 0; j < NP; j++) {
		...
		for (USI i = 0; i < NC; i++) {
			...
			An[i] = 2 / nu[j] * (tmp - aj);
			Bn[i] = 1 / nu[j] * (Bi[i] - bj);
			Znj[i] = ((bj - zj) * An[i] + ((aj + delta1 * delta2 * (3 * bj * bj + 2 * bj))
		...
		for (USI i = 0; i < NC; i++) {
			...
			for (USI k = 0; k <= i; k++) {
				...
				Cnk = P / (zj - bj) / (zj - bj) * ((zj - bj) / nu[j] * (delta(i, k) - xj[i]) - xj[i] * (Znj[k] - Bn[k]));
				Dnk = Bi[i] / bj * (Znj[k] - (Bi[k] - bj) * (zj - 1) / (nu[j] * bj));
				Gnk = (delta1 - delta2) / ((zj + delta2 * bj) * (zj + delta2 * bj)) * (Bn[k] * zj - Znj[k] * bj);
				Enk = 1 / ((delta1 - delta2) * bj * bj) * (An[k] * bj - Bn[k] * aj) * (Bi[i] / bj - 2 * tmp / aj)
					+ aj / ((delta1 - delta2) * bj) * (-Bi[i] / (bj * bj) * Bn[k] - 2 / (aj * aj) * (aj * (aik - tmp) / nu[j] - An[k] * tmp));
...
void MixtureComp::CalViscoLBC()
{
	...
	for (USI j = 0; j < NP; j++) {
		...
		if (muA[3] <= 0.18) {
			...
			muA[4] = muA[3] * (muA[3] * (muA[3] * (LBCcoef[4] * muA[3] + LBCcoef[3]) + LBCcoef[2]) + LBCcoef[1]) + LBCcoef[0];
...
void MixtureComp::CalMuPXLBC()
{
	...
	if (NP == 1) {
		...
		if (muAuxj[3] <= 0.18) {
			...
			der8J = der7J * (LBCcoef[1] + muAuxj[3] * (2 * LBCcoef[2]
				+ muAuxj[3] * (3 * LBCcoef[3] + muAuxj[3] * 4 * LBCcoef[4])));
		...
		for (USI k = 0; k < NC; k++) {
			...
			for (USI i = 0; i < NC; i++) {	
				...
				der3J += xj[i] * sqrtMWi[i] * der2IJ + delta(i, k) * sqrtMWi[k] * val2IJ;
			}
			der4J = sqrtMWi[k];
			...
			if (muAuxj[3] <= 0.18) {
				...
				der8J = der7J * (LBCcoef[1] + muAuxj[3] * (2 * LBCcoef[2]
					+ muAuxj[3] * (3 * LBCcoef[3] + muAuxj[3] * 4 * LBCcoef[4])));
				mux[bId + k] = (der3J * muAuxj[1] - muAuxj[0] * der4J) / (muAuxj[1] * muAuxj[1])
					+ 1E-4 * (4 * pow(muAuxj[4], 3) * der8J * muAuxj[2] - (pow(muAuxj[4], 4) - 1) * der6J) / (muAuxj[2] * muAuxj[2]);
			}
		}
	}
	else {
		...
		for (USI j = 0; j < NP; j++) {
			...
			if (muAuxj[3] <= 0.18) {
				muP[bId] = (der3J * muAuxj[1] - muAuxj[0] * der4J) / (muAuxj[1] * muAuxj[1])
					+ 2.05 * 1E-4 * (der7J * muAuxj[2] - muAuxj[3] * der6J) / (muAuxj[2] * muAuxj[2]);
			}
			else {
				der8J = der7J * (LBCcoef[1] + muAuxj[3] * (2 * LBCcoef[2]
...
void MixtureComp::CalVfiVfp()
{
	...
	if (NP == 1) {
		...
		for (USI i = 0; i < NC; i++) {
			...
			An[i] = 2 / nu[0] * (tmp - aj);
			Bn[i] = 1 / nu[0] * (Bi[i] - bj);
			Znij[i] = ((bj - zj) * An[i] + ((aj + delta1 * delta2 * (3 * bj * bj + 2 * bj))
				+ ((delta1 + delta2) * (2 * bj + 1) - 2 * delta1 * delta2 * bj) * zj
				- (delta1 + delta2 - 1) * zj * zj) * Bn[i]) / (3 * zj * zj + 2 * ((delta1 + delta2 - 1) * bj - 1) * zj
					+ (aj + delta1 * delta2 * bj * bj - (delta1 + delta2) * bj * (bj + 1)));
			vfi[i] = CgTP * (zj + nu[0] * Znij[i]);
		}
		Zp[0] = ((bj - zj) * aj + ((aj + delta1 * delta2 * (3 * bj * bj + 2 * bj))
			+ ((delta1 + delta2) * (2 * bj + 1) - 2 * delta1 * delta2 * bj) * zj
			- (delta1 + delta2 - 1) * zj * zj) * bj) / P / (3 * zj * zj + 2 * ((delta1 + delta2 - 1) * bj - 1) * zj
				+ (aj + delta1 * delta2 * bj * bj - (delta1 + delta2) * bj * (bj + 1)));
...
void MixtureComp::CaldXsdXp()
{
	...
	for (USI i = 0; i < NC; i++) {
		...
		for (USI m = 0; m < NP; m++) {
			for (USI n = 0; n < NC; n++) {
				MTmp[n] = vf * S[m] * (xixC[m * NC + n] * x[m][i] + delta(i, n) * xiC[m]);
			}
			MTmp += NC;
		}
	}
