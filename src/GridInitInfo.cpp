/*! \file    GridInitInfo.cpp
 *  \brief   GridInitInfo class definition
 *  \author  Shizhe Li
 *  \date    DEC/08/2022
 *
 *-----------------------------------------------------------------------------------
...
void GridInitInfo::InputParam(const ParamReservoir& rs_param)
{
    // dimensions
...
void GridInitInfo::SetupOrthogonalGrid()
{
    // x -> y -> z     
...
void GridInitInfo::CalDepthVOrthogonalGrid()
{
...
void GridInitInfo::SetupNeighborOrthogonalGrid()
{
...
void GridInitInfo::SetupCornerGrid()
{
    OCP_COORD coordTmp;
...
void GridInitInfo::SetupBasicCornerGrid(const OCP_COORD& CoTmp)
{
    dx = CoTmp.dx;
    dy = CoTmp.dy;
    dz = CoTmp.dz;
...
void GridInitInfo::SetupNeighborCornerGrid(const OCP_COORD& CoTmp)
{
    gNeighbor.resize(numGrid);
    // PreAllocate
    for (OCP_USI n = 0; n < numGrid; n++) {
...
void GridInitInfo::CalActiveGridIsoT(const OCP_DBL& e1, const OCP_DBL& e2)
{
...
void GridInitInfo::CalActiveGridT(const OCP_DBL& e1, const OCP_DBL& e2)
{
    map_Act2All.reserve(numGrid);
    map_All2Act.resize(numGrid);
...
OCP_INT GridInitInfo::GetActIndex(const USI& I, const USI& J, const USI& K) const
{
...
void GridInitInfo::GetIJKGrid(USI& i, USI& j, USI& k, const OCP_USI& n) const
{
    k = n / (nx * ny) + 1;
    j = (n - (k - 1) * nx * ny) / nx + 1;
...
void GridInitInfo::GetIJKBulk(USI& i, USI& j, USI& k, const OCP_USI& n) const
{
    GetIJKGrid(i, j, k, map_Act2All[n]);
}


void GridInitInfo::SetHexaherdronGridOrthogonal()
{
...
void GridInitInfo::SetHexaherdronGridCorner(const OCP_COORD& mycord)
{
    if (!useVTK) return;
...
void GridInitInfo::OutputBaiscInfo() const
{
    OCP_DBL depthMax = 0;
...
void GridInitInfo::CalNumDigutIJK()
{
 ...
 /*  Author              Date             Actions                              */
 /*----------------------------------------------------------------------------*/
 /*  Shizhe Li           DEC/08/2022      Create file                          */
 /*----------------------------------------------------------------------------*/