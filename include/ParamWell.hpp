...
/// WellOptParam contains all the param used for well operation. for a well, all the params
/// in it can be changed over the time.
class WellOptParam
{
public:
	...
	vector<OCP_DBL>		zi;			///< Proportion of components of injection fluid. (COMPS model only).
};
...
{
public:
	WellOptPair(USI i, string type, vector<string>& vbuf) :d(i), opt(type, vbuf) {};
	USI					d;
	WellOptParam		opt;
};
...
/// ParamWell is an internal structure used to stores the information of wells from input files.
/// It is an intermediate interface and independent of the main simulator. After all file inputting finishs,
/// the params in it will pass to corresponding modules.
class ParamWell
{
public:
	...
	/// Input the well keyword WELSPECS. WELSPECS defines wells including well name, well location,
	/// well depth and son on.
	void InputWELSPECS(ifstream& ifs);
	/// Input the well keyword COMPDAT. COMPDAT contains the information of perforations of wells, for example
	/// the location, the trans or directions.
	...
	/// Input the keyword: TSTEP. TSTEP is used to divide the simulation time and ususally
	/// the time point is critical, at which for example, the operation mode of well will change.
	/// So the params of solving equations could be adjusted correspondingly.
	void InputTSTEP(ifstream& ifs);
	/// Input the well keyword: WELTARG. WELTARG is used to change the opeartion mode of 
	/// well anytime. For example, the oil production rate is changed from 1000 stb/day to
	/// 1500 stb/day at the 100th day.
